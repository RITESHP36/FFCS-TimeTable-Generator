						...
						<tr key={row}>
							...
							{morning_col.map((column) => (
								<td
									key={column}
									id="${row}${column}"
									className=" h-40 bg-amber-200 border-2 text-xs text-center "
								>
								<div className="grid grid-rows-1  gap-1 h-full ">
									<div className={`bg-yellow-300 h-20 flex flex-col justify-center items-center ${data[`${row}${column}`] === "" ? "" : "font-semibold"}`}>
										{morning_slots[`${row}${column}`]}
										<p className={`overflow-hidden ${data[`${row}${column}`] === "" ? "" : "font-semibold"}`}>{data[`${row}${column}`]}</p>
									</div>
									<div className={`h-20 flex flex-col justify-center items-center ${data[`L${row}${column}`] === "" ? "" : "font-semibold"}`}>
							...
							{evening_col.map((column) => (
								...
								>
									<div className="grid grid-rows-1 gap-1 ">
										<div className={`bg-yellow-300 h-20 flex flex-col justify-center items-center ${data[`1${row}${column}`] === "" ? "" : "font-semibold"}`}>
											{evening_slots[`1${row}${column}`]}
											<p className={`overflow-hidden ${data[`1${row}${column}`] === "" ? "" : "font-semibold"}`}>
												{data[`1${row}${column}`]}
											</p>
										</div>
										<div className={`h-20 flex flex-col justify-center items-center ${data[`L1${row}${column}`] === "" ? "" : "font-semibold"}`}>
